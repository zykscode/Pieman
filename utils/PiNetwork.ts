import { Keypair, Server, Asset, Operation, TransactionBuilder, Memo } from 'stellar-sdk';
...
export default class PiNetwork {
  ...
  constructor(apiKey: string, walletPrivateSeed: string, options: AxiosClientOptions | null = null) {
    ...
    this.API_KEY = apiKey;
    this.myKeypair = Keypair.fromSecret(walletPrivateSeed);
  ...
  private getHorizonClient = (network: NetworkPassphrase): Server => {
  ...
  private buildA2UTransaction = async (
    ...
    const transaction = new TransactionBuilder(myAccount, {
      fee: baseFee.toString(),
      networkPassphrase: this.NETWORK_PASSPHRASE,
      timebounds: await piHorizon.fetchTimebounds(180),
    })
      .addOperation(paymentOperation)
      .addMemo(Memo.text(transactionData.paymentIdentifier))
      .build();
  ...
  private submitTransaction = async (piHorizon: Server, transaction: StellarSdk.Transaction): Promise<string> => {
